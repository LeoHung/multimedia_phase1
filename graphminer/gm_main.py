...
def gm_to_undirected(rm_multiple = True, gm_table_name=None, gm_table_undirect_name=None):
    ...
    if gm_table_undirect_name == None:
        gm_table_undirect_name = GM_TABLE_UNDIRECT

    gm_sql_table_drop_create(db_conn, gm_table_undirect_name, "src_id integer, dst_id integer, weight real")

    if rm_multiple:
        stmt = "INSERT INTO %s " % (gm_table_undirect_name) + \
                    " SELECT src_id, dst_id, AVG(weight) FROM " + \
                    " (SELECT src_id, dst_id, weight FROM %s " % (gm_table_name) + \
                    " UNION ALL" + \
                    " SELECT dst_id \"src_id\", src_id \"dst_id\", weight FROM %s) \"TAB\"" % (gm_table_name) + \
                    " GROUP BY src_id, dst_id"
    else:
        stmt = "INSERT INTO %s " % (gm_table_undirect_name) + \
                    " (SELECT src_id, dst_id, weight FROM %s " % (gm_table_name) + \
                    " UNION ALL" + \
                    " SELECT dst_id \"src_id\", src_id \"dst_id\", weight FROM %s) " % (gm_table_name)
...
#-----------------------------------------------------------------------------#
def gm_node_degrees(gm_table_name=None, dest_table_name=None):
    cur = db_conn.cursor()

    if gm_table_name == None:
        gm_table_name = GM_TABLE

    if dest_table_name == None:
        dest_table_name = GM_NODE_DEGREES

    # Create Table to store node degrees
    # If the graph is undirected, all the degree values will be the same
    print "Computing Node degrees..."

    gm_sql_table_drop_create(db_conn, dest_table_name, "node_id integer, \
                             in_degree integer, out_degree integer")

    cur.execute ("INSERT INTO %s" % dest_table_name +
                             " SELECT node_id, SUM(in_degree) \"in_degree\", SUM(out_degree) \"out_degree\" FROM " +
                             " (SELECT dst_id \"node_id\", count(*) \"in_degree\", \
                               0 \"out_degree\" FROM %s" % gm_table_name +
                             " GROUP BY dst_id" +
                             " UNION ALL" +
                             " SELECT src_id \"node_id\", 0 \"in_degree\", \
                               count(*) \"out_degree\" FROM %s" % gm_table_name +
                             " GROUP BY src_id) \"TAB\" " +
                             " GROUP BY node_id")
    db_conn.commit()
...
def gm_connected_components (num_nodes, con_comp_table_name=None, node_table_name=None, link_table_name=None):
    ...
    while True:
        ...
        diff = gm_sql_vect_diff(db_conn, con_comp_table_name, temp_table, "node_id", "node_id", "component_id", "component_id")
    ...
    cur.execute ("SELECT count(distinct component_id) FROM %s" % con_comp_table_name)
